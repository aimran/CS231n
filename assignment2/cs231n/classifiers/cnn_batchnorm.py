...
class CNN_Batchnorm(object):
  ...
  def __init__(self, input_dim=(3, 32, 32), num_filters=32, filter_size=7,
    ...
    self.params['W1'] = np.random.randn(num_filters,
                                        input_dim[0],
                                        filter_size,
    ...
    self.params['W2'] = np.random.randn(num_filters * input_dim[1] / 2 * input_dim[2] / 2,
    ...
    if use_batchnorm == True:
        ...
        self.spatial_bn_param = {'mode': 'train'}
    if self.use_dropout:
  ...
  def loss(self, X, y=None):
    ...
    if self.use_batchnorm == True:
        conv_out, conv_cache = conv_bn_relu_pool_forward(X, W1, b1, 
                                                         gamma1, beta1,
        ...
        conv_out, conv_cache = conv_relu_pool_forward(X, W1, b1, 
                                                      conv_param, pool_param)
    if self.use_dropout:
        conv_out, conv_dropout_cache = dropout_forward(conv_out, dropout_param)

    ar_out, ar_cache = affine_relu_forward(conv_out, W2, b2)

    if self.use_batchnorm == True:
        ar_out, bn_cache = batchnorm_forward(ar_out, gamma2, beta2, affine_bn_param)
    if self.use_dropout:
    ...
    if self.dropout:
        dx = dropout_backward(dx, conv_dropout_cache)
    if self.use_batchnorm == True:
        dx, dw, db, dgamma, dbeta = conv_bn_relu_pool_backward(dx, conv_cache)
        grads['gamma1'] = dgamma
        grads['beta1'] = dbeta
    else:
